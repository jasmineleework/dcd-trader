...
用法（由 auto-trader 调用）:
    python3 calc_volatility.py --price 67000 \
        --iv-annual 0.55 \
...
def calc_sell_strike(
    ...
    分级阈值：gap_pct vs 1σ(1D)
        gap_pct = (breakeven - price) / price
        one_sigma_1d = vol_24h

    - FAR (gap_pct > one_sigma_1d)：
        BTC 远低于回本，目标是最大化权利金降本，行权概率压到 10-15%
    ...
    if gap_pct > one_sigma_1d:
        tier = "far"
        strike = int(math.ceil(upper_bound / 500)) * 500 + 500
        rationale = (
    ...
    return {
        ...
        "one_sigma_1d": round(one_sigma_1d, 6),
...
def main():
    ...
    if args.mode == "sell":
        if args.breakeven is None:
            print("Error: sell 模式需要 --breakeven 参数", file=sys.stderr)
            sys.exit(1)
        strike_result = calc_sell_strike(
            price=args.price,
