...
> **退出轮（CLOSE 档）保 10% 年化下限**：整个 PUT→CALL 闭环最终卖回 USDG 时，收回金额 >= 最初投入 USDG ×(1 + 10%/365 × 持有天数 T_eff)。FAR 档（深套降本期）不受此约束。
...
2. **退出轮保 10% 年化**：CLOSE 档行权价 >= `breakeven ×(1+10%/365×T_eff)`，保证闭环收回 USDG 达标年化（FAR 档不受约束）
3. **不做市价止损**：BTC 下跌时持续通过 CALL 积累权利金，耐心等待
4. **每天交易**：与 PUT 一致，事件日不跳过
...
- **T_eff = 投入加权平均持有天数** = `Σ(各批投入USDG × 各批持有天数) / Σ各批投入USDG`
  - 单批时 T_eff = 今天 − 该批 BTC 并入日（= 对应 PUT 被行权日）
  - 各批（并入日、投入USDG）从「合并有效成本」区块的 BTC 池构成表读
- **K_target = breakeven ×(1 + 10%/365 × T_eff)**
- **行权价 = ceil(K_target / 500) × 500**（含年化目标价向上取整到 500）
- 从可用产品中选最接近且 >= 该 strike 的实际产品
- 优先 1 天产品，同档比 USDT/USDG 选 APY 高的
- 代码：`calc_volatility.py` 的 `calc_sell_strike(breakeven, hold_days=T_eff, min_annual_yield=0.10)`，年化目标可改 `min_annual_yield`

#### FAR 档（BTC 远低于回本）

**目标**：最大化权利金降本，行权概率压到 10-15%。
**尾部风险**：strike 必然 < breakeven，BTC 意外暴涨到 strike 会小幅亏损卖出（可接受）。
**不受 10% 年化约束**：FAR 档不打算这轮退出，只攒权利金降本；意外行权那一轮不保证达标年化，属可接受尾部风险。
