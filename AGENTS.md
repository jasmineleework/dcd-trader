...
**触发条件**：每次定时任务执行完毕后，若存在显著待部署资金（USDG ≥ 1,000 或 (USDT 资金账户 + USDT 简单赚币) ≥ 1,000 或 BTC ≥ 0.0001），但**未完成对应仓位的部署**，则 `touch $DCD_WORK_DIR/.idle_scan_enabled` 启用 8h 扫描。

**取消条件**：扫描后若所有待部署资金都进入 DCD 仓位（资金账户余额回到零头 < 100 **且** USDT 简单赚币 < 1,000），则 `rm -f $DCD_WORK_DIR/.idle_scan_enabled` 停用扫描，回归每日主 cron 流程。

> **USDT 简单赚币停泊**：无合规 DCD 产品时闲置 USDT 停泊进简单赚币（活期，仅 USDT，USDG 不支持 OKX 58003）吃利息；赚币 USDT 计入"待部署"，DCD 窗口一开即赎回投单。详见 `DCD策略.md` "闲置资金停泊"章节。
